...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $this->data['totalPatients'] = $this->getTotalPatients();
        $this->data['totalfollowup'] = $this->getTotalFollowup();
        $this->data['totalbaseline'] = $this->getTotalbaseline();
        $this->data['totalOpen'] = $this->getTotalPatients(17);
        $this->data['totalClose'] = $this->getTotalPatients(18);
    ...
    public function getTotalPatients_by_employee(Request $request)
    {
        $start_date = $request->startDate;
        $end_date = $request->endDate;
        $employee_id = $request->employee_id;
        if ($employee_id <> '' and $employee_id != null) {
            $openPainFilesCount = PatientPainBaselineVw::where('doctor_id', $employee_id)->where('status', 17)->count();
            $closePainFilesCount = PatientPainBaselineVw::where('doctor_id', $employee_id)->where('status', 18)->count();
        } else {
            $openPainFilesCount = PatientPainBaselineVw::where('status', 17)->count();
...
//echo $row->gender;exit;
    ...
    public function getTotalPatientsByAge(Request $request)
    {
        ...
        $sql = "SELECT
                        ...
                        FROM (SELECT TIMESTAMPDIFF(YEAR, dob,start_date ) AS age FROM pain_patient_vw) as patient
...
//echo $row->gender;exit;
    ...
    public function getOverall(Request $request)
    {
        ...
        $start_date = $request->startDate;
        $end_date = $request->endDate;
        $sources = FollowupLast::select(DB::raw('overall_status,count(*) as count'))
    ...
    public function getFollowupMonthly(Request $request)
    {
        ...
        $color = array("1" => "#FF0F00", "2" => "#FF6600", "3" => "#FF9E01", "4" => "#FCD202", "5" => "#F8FF01", "6" => "#B0DE09", "7" => "#04D215",
            "8" => "#36D7B7", "9" => "#0D8ECF", "10" => "#0D52D1", "11" => "#2A0CD0", "12" => "#8A0CCF", "13" => "#D85F7D", "15" => "#E84860",
            "16" => "#fbcebd");
        $followupSources = FollowupPatient::select(DB::raw('count(*) as count,DATE_FORMAT(follow_up_date, "%Y/%m") as duration'))
            ->whereDate('follow_up_date', '>=', $start_date)
            ->whereDate('follow_up_date', '<=', $end_date)
            ->groupBy('duration')
    ...
    public function getTop10Drugs(Request $request)
    {
        ...
        $sql_old = "SELECT DISTINCT drug_id,sum(cnt) as cnt FROM
                (SELECT DISTINCT drug_id, COUNT(*) AS cnt 
