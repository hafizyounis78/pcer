    ...
    public function draw_patient_project_table($painFile_id)
    {
        ...
        if (isset($model))
            foreach ($model as $row) {
                ...
                $html .= '<tr><td>' . $i++ . '</td>';
                $html .= '<td><a  data-toggle="modal" href="#project_modal" title="Click to see project details" onclick="show_project_details(' . $row->project_id . ');" >' . $row->project_name . ' </a></td>';
                ...
                if ($row->end_date == '') {
                    $html .= '<td>';
                    if ( auth()->user()->id != 100) {
                       // $html .= '<button title="Add symptoms" class="btn btn-success btn-icon-only btn-xs" type="button" onclick="add_project_symtomps(' . $row->project_id . ')"><i class="fa fa-stethoscope"></i></button>';
                      //  $html .= '<button title="Stop project" class="btn btn-danger btn-icon-only btn-xs" type="button" onclick="stop_project(' . $row->id . ')"><i class="fa fa-power-off"></i></button>';
                    }
                    $html .= '</td>';
      ...
      //  $model = PainFileProjectFollowupVw::where('pain_file_id', $painFile_id)->orderBy('id')->get();
        ...
        if (isset($model))
            foreach ($model as $row) {
                ...
                $html .= '<tr><td>' . $i++ . '</td>';
                $html .= '<td><a  data-toggle="modal" href="#project_modal" title="Click to see project details" onclick="show_project_details(' . $row->project_id . ');" >' . $row->project_name . ' </a></td>';
                $html .= '<td>' . $row->symptoms . '</td>';
                $html .= '<td>' . $row->created_by_name . '</td>';
                $html .= '<td>' . $row->created_at . '</td>';

                // $html .= '<td><button class="btn btn-danger btn-icon-only btn-xs" type="button" onclick="delete_symptoms(' . $row->id . ')"><i class="fa fa-power-off"></i></button></td>';
                $html .= '</tr> ';
            }
        return $html;
    }
